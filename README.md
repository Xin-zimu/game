...
- 战斗训练目标、伤害训练区，以及武器/连击/冷却/墓碑状态 HUD
- 战斗状态和墓碑随格式 5 存档恢复；V0.7–V0.9 存档均可显式迁移
...
- 敌人数量、休眠、种类和活跃状态 HUD；运行时敌人不改变格式 5 存档或格式 4 世界字节
...
游戏中按 `J` 或鼠标左键攻击，按 `E` 采集或取回墓碑，按 `Q` 在快捷栏中的可用工具/武器间循环，按 `1–8` 直接选择快捷栏；按 `C` 打开制作，按 `I` 或 `Tab` 打开背包，背包内按 `R` 整理，按 `Ctrl+S` 手动保存；按 `N` 循环切换地形/群系/气候/海拔视图，按 `B` 切换区块边界。
