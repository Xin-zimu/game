...
The generation-v3 fixture for seed `无尽边境` and chunk `(-1,-4)` is `47c1e52c4fe80f9c`. A 4,225-sample broad scan contains all nine biomes. A 96×96 full-resolution continuity fixture has more than 90% matching orthogonal edges and at most four isolated cells.

## Data/render boundary

`TerrainGenerator.generate_chunk` returns `ChunkData` containing exactly 1024 bytes for terrain, continentalness, elevation, erosion, temperature, moisture and biome maps plus a SHA-256 checksum over all fields. `ChunkRenderer` consumes that data on the main thread and writes 1024 cells to a `TileMapLayer`. Generation code never accesses the player, UI or scene tree.
